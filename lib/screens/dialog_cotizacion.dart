...
class MyDialogCotizacion extends StatefulWidget {
  final Function(String, String, String, String) onProcess;
  const MyDialogCotizacion({Key? key, required this.onProcess})
      : super(key: key);
...
class _MyDialogState extends State<MyDialogCotizacion> {
  String _periodo = '';
  String _cliente = '';
  String _agrupador = '';
  ...
  int _selectedButtonPeriodo = 0;
  int _selectedButtonStock = 21;
  int _selectedButtonLinea = 33;

  //String _selectedCity = '';

  Future<void> _setSessionPeriodo(String value) async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      _periodo = value;
    });
    await prefs.setInt('periodo', int.parse(value));
  }
  ...
  Future<void> _getSessionPeriodo() async {
    final prefs = await SharedPreferences.getInstance();
    final value = prefs.getInt('periodo');
    setState(() {
      _periodo = value.toString();
    });
    print('valor session dialog periodo $_periodo');
    if (_periodo == '1') {
      _selectedButtonPeriodo = 1;
    } else {
      _selectedButtonPeriodo = 2;
    }
    print('new valor button periodo: $_selectedButtonPeriodo');
  }
                      ...
                      Row(
                        ...
                        children: [
                          ElevatedButton(
                            onPressed: () {
                              // _periodo = '1';
                              // _setSessionPeriodo(_periodo!);
                              // setState(() {
                              //   _selectedButtonPeriodo = 1;
                              // });
                              // print('Select session $_periodo');
                              // print('Select Button $_selectedButtonPeriodo');
                            },
                            child: Text('Mes Actual',
                                style: TextStyle(
                                  color: _selectedButtonPeriodo == 1
                                      ? Colors.white
                                      : Colors.black,
                                ),
                                textAlign: TextAlign.center),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: _selectedButtonPeriodo == 1
                                  ? Colors.blue
                                  : Colors.white,
                              shape: RoundedRectangleBorder(
                          ...
                          ElevatedButton(
                            ...
                            child: Text('Mes Anterior',
                                style: TextStyle(
                                  color: _selectedButtonPeriodo == 2
                                      ? Colors.white
                                      : Colors.black,
                                ),
                                textAlign: TextAlign.center),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: _selectedButtonPeriodo == 2
                                  ? Colors.blue
                                  : Colors.white,
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(5.0),
                          ...
                          showDialog(
                            ...
                            builder: (BuildContext context) {
                              return AlertDialog(
                                ...
                                actions: [
                                  TextButton(
                                    onPressed: widget.onProcess(_cliente,
                                        _agrupador, _subagrupador, _periodo),
                                    /*() {
